...
@interface WWPopOverViewController : UIViewController
...
@implementation WWPopOverViewController
...
@implementation WWGeneric (Application)
...
+ (UIWindow *)popOverWindow {
    ...
    dispatch_once(&onceToken, ^{
        ...
        popOverWindow.rootViewController = [[WWPopOverViewController alloc] initWithNibName:nil bundle:nil];
    });
...
+ (void)addViewToPopOverWindow:(UIView *)view {
    WWPopOverViewController *vc = (WWPopOverViewController *)[WWGeneric popOverWindow].rootViewController;
    ...
    [WWGeneric popOverWindow].hidden = NO;
...
+ (void)removeViewFromPopOverWindow:(UIView *)view {
    ...
    if (vc.view.subviews.count == 0) {
        [WWGeneric popOverWindow].hidden = YES;
    }
}


+ (WWAppDelegate *)appDelegate {
    return (WWAppDelegate *)[UIApplication sharedApplication].delegate;
}
